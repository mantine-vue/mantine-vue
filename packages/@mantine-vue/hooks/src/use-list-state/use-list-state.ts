...
export type UseListStateReturnValue<T> = [Ref<T[]>, UseListStateHandlers<T>]
...
export function useListState<T>(initialValue: T[] | (() => T[]) = []): UseListStateReturnValue<T> {
  ...
  const setState = (value: T[] | ((current: T[]) => T[])) => {
    state.value =
      typeof value === 'function' ? (value as (current: T[]) => T[])(state.value) : value
  }
  ...
  const handlers: UseListStateHandlers<T> = {
    setState,
    append,
    prepend,
    insert,
