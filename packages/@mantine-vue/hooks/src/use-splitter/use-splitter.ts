...
function applyConstraints(
  ...
  const collapsedResult = checkCollapse(sizes, panels, handleIndex, delta)
  if (collapsedResult) {
...
export function useSplitter<T extends HTMLElement = any>(
  ...
  const [currentSizes, setCurrentSizesRaw] = useUncontrolled<number[]>({
  ...
  const activeHandle = ref(-1) as Ref<number>

  const internalState: SplitterInternalState = createInitialInternalState()
  let containerNode: T | null = null
  let documentController: AbortController | null = null
  let frame = 0
  let preCollapseSizes: number[] = [...defaultSizes]

  const collapsed = computed(() => currentSizes.value.map((size) => size === 0)) as Ref<boolean[]>

  const updateSizes = (newSizes: number[]) => {
    setCurrentSizesRaw(newSizes)
  }
  ...
  const collapsePanel = (panelIndex: number) => {
    ...
    const sizesNow = currentSizes.value
    if (sizesNow[panelIndex] === 0) {
      return
    }

    preCollapseSizes = [...sizesNow]
    const newSizes = [...sizesNow]
  ...
  const expandPanel = (panelIndex: number) => {
    ...
    const restoreSize = preCollapseSizes[panelIndex] || panels[panelIndex].defaultSize
    const newSizes = [...sizesNow]
  ...
  const toggleCollapsePanel = (panelIndex: number) => {
    if (currentSizes.value[panelIndex] === 0) {
  ...
  const reset = (handleIndex: number) => {
    ...
    const next = applyAdjacentOnly(prev, panels, beforeIdx, targetBefore - prev[beforeIdx])
  ...
  const getHandleRefCallback = (handleIndex: number): ((node: HTMLElement | null) => void) => {
    ...
    const callback = (node: HTMLElement | null) => {
      ...
      handleElementControllers.set(handleIndex, elementController)
      ...
      const flushResize = (pointerEvent: PointerEvent) => {
        ...
        const isRtl = isHorizontal && dir === 'rtl'
        ...
        const prevSizes = currentSizes.value
      ...
      const onPointerUp = (event: PointerEvent) => {
        ...
        onResizeEnd?.(finishedHandle, [...currentSizes.value])
      }
      ...
      const onPointerDown = (event: PointerEvent) => {
        ...
        internalState.handleIndex = handleIndex
        internalState.startPointer = pointerPos
        internalState.containerSize = containerSize
        internalState.startSizes = [...currentSizes.value]
        internalState.preCollapseSizes = [...preCollapseSizes]
    ...
    handleRefCallbacks.set(handleIndex, callback)
    return callback
  }
  ...
  const getHandleProps = (input: { index: number }) => {
    ...
    const beforeSize = currentSizes.value[index] ?? 0
    ...
    return {
      ...
      onKeyDown: (event: KeyboardEvent) => {
        ...
        switch (event.key) {
          ...
          case 'Home': {
            delta = -(currentSizes.value[index] - getMin(beforePanel))
            break
          }
          case 'End': {
            delta = getMax(beforePanel) - currentSizes.value[index]
            break
          }
          case 'Enter': {
            ...
            if (beforeCollapsible && currentSizes.value[index] <= currentSizes.value[index + 1]) {
        ...
        if (delta !== 0) {
          const newSizes = applyConstraints(currentSizes.value, panels, index, delta, redistribute)
          emitCollapseTransitions(
            currentSizes.value,
            newSizes,
            [index, index + 1],
  ...
  onBeforeUnmount(() => {
    documentController?.abort()
    documentController = null
    handleElementControllers.forEach((controller) => controller.abort())
    handleElementControllers.clear()
    cancelAnimationFrame(frame)

    if (internalState.isDragging) {
