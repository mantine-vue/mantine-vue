...
export const useNprogress = (store: NprogressStore): Readonly<Ref<NprogressState>> => store.value
...
export function startNavigationProgressAction(store: NprogressStore) {
  updateNavigationProgressStateAction(
    (state) => ({ progress: getIntervalProgressValue(state.progress), mounted: true }),
    store,
  )
  ...
  updateNavigationProgressStateAction((state) => {
    ...
    const interval = window.setInterval(() => {
      updateNavigationProgressStateAction(
        (current) => ({ progress: getIntervalProgressValue(current.progress), mounted: true }),
        store,
      )
    }, state.stepInterval)
