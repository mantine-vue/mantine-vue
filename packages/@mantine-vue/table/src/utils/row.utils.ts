...
export const getMVT_Rows = <TData extends MVT_RowData>(
...
export const getCanRankRows = <TData extends MVT_RowData>(table: MVT_TableInstance<TData>) => {
  const {
    getState,
    options: {
...
export const getIsRankingRows = <TData extends MVT_RowData>(table: MVT_TableInstance<TData>) => {
  const { globalFilter, sorting } = table.getState()

  return getCanRankRows(table) && globalFilter && !Object.values(sorting).some(Boolean)
}

export const getIsRowSelected = <TData extends MVT_RowData>({
  row,
  table,
}: {
  row: MVT_Row<TData>
...
export const getMVT_RowSelectionHandler =
  ...
  (event: Event, value?: boolean) => {
    ...
    if (
      ...
      const rows = getMVT_Rows(table, true)

      const lastIndex = rows.findIndex((r) => r.id === lastSelectedRowId.value)
...
export const getMVT_SelectAllHandler =
  <TData extends MVT_RowData>({ table }: { table: MVT_TableInstance<TData> }) =>
  (event: Event, value?: boolean, forceAll?: boolean) => {
    const {
    ...
    if (selectAllMode === 'all' || forceAll) {
      table.toggleAllRowsSelected(value ?? (event.target as HTMLInputElement).checked)
    } else {
      table.toggleAllPageRowsSelected(value ?? (event.target as HTMLInputElement).checked)
    }
    if (enableRowPinning && rowPinningDisplayMode?.includes('select')) {
