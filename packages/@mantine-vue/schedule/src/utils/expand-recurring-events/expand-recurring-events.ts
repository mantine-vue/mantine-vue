...
function resolveRRule(ns: Record<string, unknown>): typeof rruleAll.RRule {
  const interop = (ns.default ?? ns) as typeof rruleAll
  return interop.RRule
}
const RRule = resolveRRule(rruleAll as unknown as Record<string, unknown>)
...
function getOccurrenceStartsInRange(
  ...
  try {
    ...
    return rule.between(searchStart, toWallClockDate(rangeEnd), true).slice(0, expansionLimit)
  } catch {
    return []
  }
}
...
function createGeneratedInstance(
  ...
  const { recurrence: _recurrence, ...eventWithoutRecurrence } = event
