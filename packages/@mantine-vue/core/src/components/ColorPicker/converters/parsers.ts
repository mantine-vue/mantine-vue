import type { ColorFormat, HslaColor, HsvaColor, RgbaColor } from '../ColorPicker.types'
export function round(number: number, digits = 0, base = 10 ** digits) {
  return Math.round(base * number) / base
}
function hslaToHsva({ h, s, l, a }: HslaColor): HsvaColor {
  const ss = s * ((l < 50 ? l : 100 - l) / 100)
  return { h, s: ss > 0 ? ((2 * ss) / (l + ss)) * 100 : 0, v: l + ss, a }
}
const angleUnits: Record<string, number> = { grad: 360 / 400, turn: 360, rad: 360 / (Math.PI * 2) }
...
export function parseHexa(color: string): HsvaColor {
  const hex = color[0] === '#' ? color.slice(1) : color
  const alpha = (value: string) => round(parseInt(value, 16) / 255, 3)
  return hex.length === 4
    ? { ...parseHex(hex.slice(0, 3)), a: alpha(hex[3] + hex[3]) }
    : { ...parseHex(hex.slice(0, 6)), a: alpha(hex.slice(6, 8)) }
}
const RGB_REGEXP =
...
const VALIDATION: Record<ColorFormat, RegExp> = {
  hex: /^#?([0-9A-F]{3}){1,2}$/i,
  hexa: /^#?([0-9A-F]{4}){1,2}$/i,
...
export function isColorValid(color: string) {
  return (
    typeof color === 'string' && Object.values(VALIDATION).some((rule) => rule.test(color.trim()))
  )
}
export function parseColor(color: string): HsvaColor {
  ...
  for (const [format, rule] of Object.entries(VALIDATION))
    if (rule.test(value)) return PARSERS[format as ColorFormat](value)
