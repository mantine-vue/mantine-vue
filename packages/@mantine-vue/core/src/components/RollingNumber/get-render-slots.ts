...
export function getRenderSlots({
  ...
  if (current.hasDecimal || previous.hasDecimal) {
    slots.push({ type: 'char', key: 'dec', char: decimalSeparator, empty: !current.hasDecimal })
  }

  for (let i = 0; i < maxFracLen; i += 1) {
...
export function getRenderSlotsFromValues({
  ...
  return getRenderSlots({ current, previous, prefix, suffix, decimalSeparator, thousandSeparator })
}
