...
function controlProps(date: DateStringValue) {
  const external = props.getYearControlProps?.(date) || {}
  ...
  const boundaries = [start, previewEnd].sort((a, b) => (dayjs(a).isAfter(b, 'year') ? 1 : -1))
  return {
    inRange: isInRangeGranular(date, [start, previewEnd], 'year'),
    firstInRange: dayjs(date).isSame(boundaries[0], 'year'),
    lastInRange: dayjs(date).isSame(boundaries[1], 'year'),
    ...external,
  }
}
...
<template>
  <div v-if="props.presets?.length" :class="classes.datePickerRoot">
    ...
    <Calendar
      ...
      :value="value"
      :date="calendarDate"
      default-level="decade"
      min-level="decade"
      :get-year-control-props="controlProps"
      @date-change="setCalendarDate"
      @year-select="select"
      @year-mouse-enter="hover"
