import { ContainerDemos } from '@/demos'
...
`Container` supports the `strategy="grid"` prop which enables more features.
...
Note that the `size` prop does not make `max-width` responsive — when set to `lg` it will always
be `lg` regardless of screen size.
...
You can customize existing `Container` sizes and add new ones with CSS variables on the theme:

<Demo data={ContainerDemos.sizes} />

## Responsive max-width

To make `Container` `max-width` responsive, use Styles API to set `classNames`. For example,
you can add a `responsive` size that will make the `Container` `max-width` different depending
on screen size:
