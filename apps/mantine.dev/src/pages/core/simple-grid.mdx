import { SimpleGridDemos } from '@/demos'
...
the [Grid](/core/grid) component instead.
...
`sm`, `md`, `lg` and `xl` keys, and values from those keys will be applied according to current
viewport width.
...
do not use the container queries option.
