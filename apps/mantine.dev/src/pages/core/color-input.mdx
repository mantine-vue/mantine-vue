import { ColorInputDemos } from '@/demos'
...
Set `fullWidth` to make the dropdown match the input width and stretch the color picker inside it.

<Demo data={ColorInputDemos.fullWidth} />
...
Note that by default, `ColorInput` has a color preview in the left section and an eye dropper button
in the right section. You can replace these elements with any node using the `leftSection`
