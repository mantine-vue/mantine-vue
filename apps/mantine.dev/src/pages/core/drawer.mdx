...
```vue
<template>
  <Drawer position="right" size="xl" :opened="true" @close="() => {}">
    <!-- Drawer content -->
  </Drawer>
</template>
```
...
```vue
<template>
  <Drawer title="Drawer label" :opened="true" @close="() => {}" />
</template>
```
...
```vue
<template>
  <Drawer :close-button-props="{ 'aria-label': 'Close drawer' }" :opened="true" @close="() => {}" />
</template>
```
