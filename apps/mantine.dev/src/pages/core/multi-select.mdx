import { MultiSelectDemos } from '@/demos'
...
checked state. The function must return a VNode.

<Demo data={MultiSelectDemos.renderOption} />

## renderPill

The `renderPill` callback allows you to customize pill rendering. The function receives the option, value, onRemove and disabled props. It must return a VNode.
...
<template>
  <MultiSelect :data="[]" clearable :clear-button-props="{ 'aria-label': 'Clear input' }" />
</template>
```
