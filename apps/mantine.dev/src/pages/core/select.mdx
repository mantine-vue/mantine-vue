...
const value = ref<{ value: string; label: string } | null>(null)

function handleChange(_value: string | null, option: { value: string; label: string }) {
  value.value = option
}
</script>
...
<template>
  <Select
    ...
    :model-value="value ? value.value : null"
    @change="handleChange"
  />
</template>
```
...
<template>
  <Select searchable v-model:search-value="searchValue" :data="[]" />
</template>
```
...
checked state. The function must return a Vue node.
...
<template>
  <Select
    ...
    :combobox-props="{ middlewares: { shift: { padding: 0 } } }"
  />
</template>
```
...
<template>
  <Select :data="[]" clearable :clear-button-props="{ 'aria-label': 'Clear input' }" />
</template>
```
