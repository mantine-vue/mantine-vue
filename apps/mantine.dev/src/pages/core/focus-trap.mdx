import { FocusTrapDemos, ModalDemos } from '@/demos'
...
`FocusTrap` is a component implementation of the `use-focus-trap` hook.
It is used in all Mantine components that require focus trap (`Modal`, `DatePicker`, `Popover`, etc.).
