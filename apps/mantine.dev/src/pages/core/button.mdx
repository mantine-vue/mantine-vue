...
import { StylesDemos, ThemingDemos } from '@/demos'
...
- `&:disabled` is used to style the button when the `disabled` prop is set and
  also when the button is disabled by the parent component.
- `&[data-disabled]` is used to style the button when it is not actually disabled but should look like
  it is (for example, when used with a [Tooltip](/core/tooltip) or as a link).

<Demo data={ButtonDemos.disabledStyles} />

## Disabled button with Tooltip

If you need to use a [Tooltip](/core/tooltip) with a disabled `Button`, set the `data-disabled` prop on the
`Button` instead of `disabled`. It is also required to change the `onClick` event handler to
...
When the `loading` prop is set, the `Button` will be disabled and a [Loader](/core/loader) with overlay will be rendered
in the center of the button. [Loader](/core/loader) color depends on the `Button` variant.
...
To add new `Button` variants, use the [data-variant](/styles/variants-sizes) attribute.
