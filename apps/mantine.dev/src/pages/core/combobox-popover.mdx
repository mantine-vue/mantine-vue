import { ComboboxPopoverDemos } from '@/demos'
...
Set `:allow-deselect="false"` to prevent this behavior.
...
You can also provide this content through the `#nothingFound` named slot:
...
Use `renderOption` prop to customize option rendering. Alternatively, you can use the
`#option` named slot, which receives the same `{ option, checked }` payload:
