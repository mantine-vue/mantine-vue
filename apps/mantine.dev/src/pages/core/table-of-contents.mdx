import { TableOfContentsDemos } from '@/demos'
...
You can pass options down to the hook using the `scrollSpyOptions` prop.
...
before the component is mounted (for example during server-side rendering),
...
const reinitializeRef = ref<(() => void) | null>(null)

onMounted(() => {
  // Call reinitialize whenever DOM headings change
  reinitializeRef.value?.()
})
</script>

<template>
  <TableOfContents :reinitialize-ref="reinitializeRef" />
</template>
```

<StylesApiSelectors component="TableOfContents" />
