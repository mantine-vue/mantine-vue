import { HighlightDemos } from '@/demos'
...
Pass the text as the default slot and specify which substring(s) to highlight with the `highlight` prop.
...
which accepts an object with styles:

<Demo data={HighlightDemos.styles} />

## Text props

Highlight is based on the [Text](/core/text/) component — all Text props are available.
