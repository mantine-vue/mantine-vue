import { TreeSelectDemos } from '@/demos'
...
<template>
  <!-- Single mode -->
  <TreeSelect :data="[]" v-model="value" />

  <!-- Multiple or checkbox mode -->
  <TreeSelect :data="[]" mode="multiple" v-model="multiValue" />
</template>
```
...
Set `:with-lines="false"` to disable them:
...
you can listen to the `dropdown-close` and `dropdown-open` events to react to dropdown opened state changes.
...
<template>
  ...
  <TreeSelect :data="data" :default-expanded-values="['fruits', 'vegetables']" />

  <!-- Expand all nodes by default -->
  <TreeSelect :data="data" default-expand-all />

  <!-- Controlled expanded state -->
  <TreeSelect :data="data" :expanded-values="expandedValues" @expanded-change="setExpandedValues" />
</template>
```
...
`TreeSelect` will not show suggestions and will not emit the `change` event.
...
```vue
<template>
  <TreeSelect :data="[]" clearable :clear-button-props="{ 'aria-label': 'Clear input' }" />
</template>
```
