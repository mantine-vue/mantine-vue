import { TagsInputDemos } from '@/demos'
...
<template>
  <TagsInput v-model:search-value="searchValue" :data="[]" />
</template>
```
...
The function must return a Vue node.
...
If you use a custom pill renderer with the `renderPill` slot, spread the `reorderProps` from
...
<template>
  <TagsInput with-pills-reorder>
    <template #renderPill="{ value, onRemove, reorderProps }">
      <div v-bind="reorderProps">
        {{ value }}
...
<template>
  <TagsInput :data="[]" clearable :clear-button-props="{ 'aria-label': 'Clear input' }" />
</template>
```
