...
import { Combobox, useCombobox } from '@mantine-vue/core'
...
interface UseComboboxOptions {
  ...
  /** Controlled `dropdownOpened` state, can be a ref, a getter or a plain value */
...
You can import the `UseComboboxOptions` type from the `@mantine-vue/core` package:

```ts
import type { UseComboboxOptions } from '@mantine-vue/core'
```
...
interface ComboboxStore {
   ...
   *  Required with searchable components to update selected option index
...
You can import `ComboboxStore` type from `@mantine-vue/core` package:

```ts
import type { ComboboxStore } from '@mantine-vue/core'
```
...
```vue
<script setup lang="ts">
import { Button, Combobox, useCombobox } from '@mantine-vue/core'
...
<template>
  <Combobox>
    <Combobox.Target>
      <Button @click="combobox.openDropdown()">Open dropdown</Button>
    </Combobox.Target>
...
`Combobox.Target` marks its child as a target for the dropdown and sets `aria-*` attributes
and keyboard event listeners on it.

`Combobox.Target` requires a single child element or component. The child component
must accept a `ref` and forward `...attrs`. You can use any Mantine component as a target without
...
`updateSelectedOptionIndex` handler is required to be called when the options list changes.
Usually, the options list changes when options are filtered based on the search query. In this case,
you need to call `updateSelectedOptionIndex` in the `change` handler of the search input.

Example of using `updateSelectedOptionIndex` handler in a searchable select component:

<Demo data={ComboboxDemos.searchableSelect} />

## Search input

If you prefer a search input inside the dropdown, use `Combobox.Search` component.
...
Use the `combobox.selectFirstOption` function to select the first option. It is useful
if you want to select the first option while the user is searching for options in the list.
...
Set the `active` prop on `Combobox.Option` component to mark it as active.
By default, an active option does not have any styles, you can use the `data-combobox-active`
[data attribute](/styles/data-attributes) to style it.

`combobox.selectActiveOption` function selects the active option. Usually, it is called
...
Render `Combobox.Option` components inside `Combobox.Group` to create an options group.
The `Combobox.Group` label is automatically hidden if the group does not have any
children.
...
Set the `max-height` style on either `Combobox.Dropdown` or `Combobox.Options` to make the
options list scrollable. You can use the `mah` [style prop](/styles/style-props) to set
`max-height`.
...
Set `floating-height="viewport"` to make the dropdown grow to fill the available vertical
...
Set the `hidden` prop on `Combobox.Dropdown` to hide the dropdown. For example,
...
To control the dropdown opened state, pass `opened` to the `useCombobox` hook. It accepts a
plain value, a `ref`, or a getter function:
...
you can control the dropdown position with the `position` prop and disable Floating UI
middlewares with the `middlewares` prop:

<Demo data={ComboboxDemos.dropdownPosition} />

## Without dropdown

You can use `Combobox` without a dropdown. To do so, use `Combobox.EventsTarget` instead
of `Combobox.Target`:
...
`useVirtualizedCombobox` hook can be used to create comboboxes with a virtualized options list.
...
component state and pass them to the hook.

`useVirtualizedCombobox` does not depend on any specific virtualization library – pair it with
whichever virtualization approach you use in your application.

Hook options:

```ts
interface UseVirtualizedComboboxOptions {
  /** Default value for `dropdownOpened`, `false` by default */
  defaultOpened?: boolean
  ...
  /** Total number of options in the virtualized list, can be a ref or a getter. Required for proper keyboard navigation and index calculations. */
  ...
  /** Current selected option index, can be a ref or a getter. Must be controlled by the parent component. */
  selectedOptionIndex: MaybeRefOrGetter<number>

  /** Callback to update the selected option index. Called when the user navigates or selects options. */
  setSelectedOptionIndex: (index: number) => void

  /** Currently active/highlighted option index, can be a ref or a getter. Used to determine which option to select when selectActiveOption is called. */
...
You can import the `useVirtualizedCombobox` hook and the `UseVirtualizedComboboxOptions` type from
the `@mantine-vue/core` package:

```ts
import { useVirtualizedCombobox } from '@mantine-vue/core'
import type { UseVirtualizedComboboxOptions } from '@mantine-vue/core'
```
