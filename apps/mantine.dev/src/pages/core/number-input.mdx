...
<template>
  <form @submit="handleSubmit">
    <NumberInput label="Enter quantity" name="quantity" defaultValue="1" :min="1" :max="100" />
...
- **`onChange`**: Receives a simplified value (`number | string` in default mode, `bigint | string` in BigInt mode). This is the recommended callback for most use cases.

- **`onValueChange`**: Receives the full payload including `floatValue`, `formattedValue`, `value`, and metadata about the change source. Use this when you need access to the formatted value or change metadata.
...
<template>
  <NumberInput
    prefix="$"
    thousandSeparator=","
    @change="(value) => console.log('Simple value:', value)"
    @valueChange="(payload) => console.log('Full payload:', payload)"
  />
</template>
```
...
<template>
  <NumberInput
    :min="0"
    :max="100"
    @minReached="() => console.log('Minimum value reached')"
    @maxReached="() => console.log('Maximum value reached')"
  />
</template>
```
...
```vue
<template>
  <NumberInput selectAllOnFocus :defaultValue="100" />
</template>
```
...
`stepHoldInterval` can be a fixed number (ms) or a function `(elapsedMs) => number` to vary speed over time:
