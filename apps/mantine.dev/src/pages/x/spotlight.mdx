import { SpotlightDemos } from '@/demos'
...
<InstallScript packages="@mantine-vue/spotlight" />
...
`Spotlight` is based on the [Modal](/core/modal) component and supports most of its props.

<Demo data={SpotlightDemos.usage} />

## Actions

The `@mantine-vue/spotlight` package exports an object with actions that can be used to control the spotlight:

```ts
import { spotlight } from '@mantine-vue/spotlight'
...
You can also import actions directly from the `@mantine-vue/spotlight` package if you prefer this syntax:

```ts
import { closeSpotlight, openSpotlight, toggleSpotlight } from '@mantine-vue/spotlight'
...
// You can import `firstStore`/`firstSpotlight` and `secondStore`/`secondSpotlight`
// anywhere in your application and use `open`, `close` and `toggle` actions
// to control spotlight the same way as with the default `spotlight` object
const [firstStore, firstSpotlight] = createSpotlight()
const [secondStore, secondSpotlight] = createSpotlight()
</script>

<template>
  <Button @click="firstSpotlight.open()">Open first spotlight</Button>
  <Button @click="secondSpotlight.open()">Open second spotlight</Button>
...
`Spotlight` uses a `window` keydown listener to handle keyboard shortcuts.
...
<template>
  ...
  <!-- Disable shortcut -->
  <Spotlight :shortcut="null" :actions="[]" />
</template>
```
...
<Demo data={SpotlightDemos.fuzzySearch} />
...
- When the `scrollable` prop is set, the actions list height will always equal the value of the `maxHeight` prop
  ...
  by the [ScrollArea](/core/scroll-area) component.
...
While `Spotlight` is open, page scroll is locked: `data-scroll-locked` is set on `<body>` together with
a `--removed-body-scroll-bar-size` CSS variable holding the width of the scrollbar that was hidden.
If you have fixed positioned elements in your application, use this variable to offset them so they
do not shift when the scrollbar disappears:
...
body[data-scroll-locked] .fixed-element {
