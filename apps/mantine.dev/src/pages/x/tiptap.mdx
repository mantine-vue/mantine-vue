...
<InstallScript packages="@mantine-vue/tiptap @mantine-vue/core @mantine-vue/hooks @tiptap/vue-3 @tiptap/pm @tiptap/extension-link @tiptap/starter-kit" />
...
The `@mantine-vue/tiptap` package provides a UI for [Tiptap](https://tiptap.dev/). The `RichTextEditor` component
...
with the [useEditor composable](https://tiptap.dev/docs/editor/getting-started/install/vue3).
...
To control the editor state, create a wrapper component and pass an `onUpdate` handler
to the `useEditor` composable:

```vue
<script setup lang="ts">
import { useEditor } from '@tiptap/vue-3'
import StarterKit from '@tiptap/starter-kit'
import { RichTextEditor as MantineRichTextEditor } from '@mantine-vue/tiptap'
...
The `@mantine-vue/tiptap` package provides a custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for the `Ctrl + K` keyboard shortcut to work:

```vue
<script setup lang="ts">
// Use Link extension exported from the @mantine-vue/tiptap package
import { useEditor } from '@tiptap/vue-3'
...
- `RichTextEditor.ColorPicker` – allows you to pick colors from given predefined color swatches and with the [ColorPicker](/core/color-picker) component
...
Install and register `TableKit`, which bundles the required table, row, header and cell nodes:

<InstallScript packages="@tiptap/extension-table" />

<Demo data={TipTapDemos.table} />

Available controls:

- `TableInsert` opens a grid for selecting dimensions and supports `maxColumns`, `maxRows`,
  `withHeaderRow` and `popoverProps`.
- `TableDelete` removes the table.
- `TableColumnBefore`, `TableColumnAfter`, and `TableColumnDelete` manage columns.
- `TableRowBefore`, `TableRowAfter`, and `TableRowDelete` manage rows.
- `TableToggleHeaderRow` and `TableToggleHeaderColumn` toggle headers.
- `TableMergeCells` and `TableSplitCell` manage selected cells.

Controls are disabled when the extension is absent. Controls other than `TableInsert` also require
the cursor to be inside a table.

## Details

Install and register `Details`, `DetailsSummary`, and `DetailsContent`:

<InstallScript packages="@tiptap/extension-details" />

<Demo data={TipTapDemos.details} />

`RichTextEditor.Details` wraps the current block in a details node or removes the surrounding node.
It is disabled when the extension is unavailable.

## Invisible characters

Install the extension with `visible: false` when marks should initially be hidden:

<InstallScript packages="@tiptap/extension-invisible-characters" />

<Demo data={TipTapDemos.invisibleCharacters} />

`RichTextEditor.InvisibleCharacters` toggles spaces, paragraph breaks and hard-break markers. Its
active state follows extension storage and it is disabled when the command is unavailable.

## Removing paragraph wrappers

Tiptap table cells and list items allow block content by default, which produces markup such as
`<td><p>Text</p></td>`. Override a node schema with `content: 'inline*'` only when a cell must be
single-line:
...
Inline-only cells cannot contain multiple paragraphs or other block nodes; keep the default schema
unless that restriction is intended.
...
<template>
  <RichTextEditor :editor="editor" :with-typography-styles="false">
    <RichTextEditor.Content />
  </RichTextEditor>
</template>
```
...
exported from `@tiptap/vue-3/menus` with any `RichTextEditor` controls. The bubble menu will appear near a selection of text:
...
exported from `@tiptap/vue-3/menus` with any `RichTextEditor` controls. The floating menu will appear in an empty line:
...
control the `top` property with `stickyOffset`. For example, on this documentation
website there is a header with `var(--docs-header-height)` height, so the usage demo above
sets `stickyOffset="var(--docs-header-height)"` to make the sticky position work correctly with the fixed positioned element.

## Editor context

Use the `useRichTextEditorContext` composable to get the [Editor](https://tiptap.dev/api/editor) from
the context. This composable can be used to create a custom control or run any operations supported
...
import { useRichTextEditorContext } from '@mantine-vue/tiptap'

const Demo = defineComponent(() => {
  const { editor } = useRichTextEditorContext()
  return () =>
    h(Button, { onClick: () => editor?.chain().focus().toggleBold().run() }, () => 'Make bold')
})
```
...
props supported by the native `button` element and has an `active` prop to indicate active state.
...
```ts
// RichTextEditorLabels type can be imported from @mantine-vue/tiptap package
...
Default labels (can be imported from `@mantine-vue/tiptap` package):

```ts
import { RichTextEditorLabels } from '@mantine-vue/tiptap'
