import { CodeHighlightDemos } from '@/demos'
...
<InstallScript packages="@mantine-vue/code-highlight" />
...
`@mantine-vue/code-highlight` package does not depend on any specific code highlighting library.
...
The Shiki adapter is recommended if you need to highlight advanced TypeScript (generics, JSX/TSX nested in props) or CSS code (custom syntaxes, newest features).
The Shiki adapter is used for all code highlighting in this documentation.
...
import { CodeHighlightAdapterProvider, createShikiAdapter } from '@mantine-vue/code-highlight'
...
const app = createApp({
  render: () =>
    h(MantineProvider, null, () =>
      h(CodeHighlightAdapterProvider, { adapter: shikiAdapter }, () => h(App)),
    ),
})
...
To load Shiki grammars on demand, pass `resolveLanguage` to `createShikiAdapter`. The resolver
can return a language registration or a dynamic import accepted by Shiki's `loadLanguage`.
`CodeHighlight` requests its current language automatically.

Set `withFirstLineIndentation` when the indentation before the first code line is meaningful.
The same normalized value is highlighted, displayed, numbered, and copied.
...
import { createApp, h } from 'vue'
import { MantineProvider } from '@mantine-vue/core'
import { CodeHighlightAdapterProvider, createHighlightJsAdapter } from '@mantine-vue/code-highlight'
...
const app = createApp({
  render: () =>
    h(MantineProvider, null, () =>
      h(CodeHighlightAdapterProvider, { adapter: highlightJsAdapter }, () => h(App)),
    ),
})
...
```ts
import { type CodeHighlightAdapter, stripShikiCodeBlocks } from '@mantine-vue/code-highlight'
...
You can use any Vue node as tab icon. The example below uses icons from the
`@phosphor-icons/vue` package, but you can use any other icons library or custom icons:
...
to assign icons based on file name. `getFileIcon` accepts a file name and must return a Vue node
or `null`.
