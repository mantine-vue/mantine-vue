import { CarouselDemos } from '@/demos'
...
<InstallScript packages="embla-carousel@^8.5.2 embla-carousel-vue@^8.5.2 @mantine-vue/carousel" />
...
`@mantine-vue/carousel` package is based on [embla carousel](https://www.embla-carousel.com/), using
[embla-carousel-vue](https://www.embla-carousel.com/get-started/vue/) instead of the React bindings:
...
You can replace default next/previous controls icons with any content by passing a vnode
to the `nextControlIcon`/`previousControlIcon` props:
...
Set the `plugins` prop to enhance the carousel with [embla plugins](https://www.embla-carousel.com/docs/plugins).
Note that plugins are not installed with the `@mantine-vue/carousel` package and you will need to
install them separately. Embla plugins are framework-agnostic, so the same plugins used with React
also work here.

Example with [autoplay plugin](https://www.embla-carousel.com/docs/plugins/autoplay):
