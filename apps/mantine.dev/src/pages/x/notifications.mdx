import { NotificationDemos, NotificationsDemos } from '@/demos'
...
<InstallScript packages="@mantine-vue/notifications" />
...
- It is required to render the `Notifications` component inside [MantineProvider](/theming/mantine-provider/)
...
The `@mantine-vue/notifications` package exports a `notifications` object with the following functions:
...
All functions can be imported from the `@mantine-vue/notifications` package and can be used in any part of your application:
...
- `priority` – controls which notifications occupy visible slots when `limit` is reached; higher values are shown first
...
<template>
  <Notifications :allow-drag-dismiss="false" :allow-scroll-dismiss="false" />
</template>
```
...
<template>
  <Notifications position="top-right" :z-index="1000" />
</template>
```
...
Notifications use insertion order when priorities are equal. Set `priority` in `show` or `update`
to promote urgent items when the visible limit is reached. Priority is applied independently per
position. A high-priority notification can move a visible lower-priority item back to the queue.
...
<template>
  <Notifications pause-reset-on-hover="notification" />
</template>
```

## Custom notification rendering

Use `renderNotification` on `Notifications`, or on an individual notification, to replace the
default `Notification` content while retaining transitions and dismiss interactions. Returning
`null` from a per-notification override uses the global renderer.

<Demo data={NotificationsDemos.renderNotification} />

## Stacked layout

Set `layout="stacked"` to collapse notifications at each position. The stack expands on hover,
keyboard focus or touch. Hidden items are inert and their auto-close timers are paused until the
stack is expanded.
...
The hook returns a ref to an object with `notifications` and `queue` arrays. The `notifications`
