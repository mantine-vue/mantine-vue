import { WeekViewDemos } from '@/demos'
...
Control the date displayed by WeekView externally using the `date` prop and the `dateChange` event.
...
Set `:with-weekend-days="false"` to hide Saturday and Sunday columns.

<Demo data={WeekViewDemos.withoutWeekendDays} />

## Without week number

Set `:with-week-number="false"` to hide the week number in the top-left corner.
...
`@mantine-vue/schedule` works with timezone-agnostic `YYYY-MM-DD HH:mm:ss` strings and does not
...
see the indicator and the time bubble move to the matching day and position:

<Demo data={WeekViewDemos.timezone} />

## Without all-day slots

Set `:with-all-day-slots="false"` to hide the all-day events section at the top.
...
Set `:with-header="false"` to hide the header controls.

<Demo data={WeekViewDemos.withoutHeader} />

## View change callback

Listen for the `viewChange` event to be notified when the user selects a different view from the header dropdown.
...
Set `:with-header="false"` on the view and compose the header externally.
...
(the `timeSlotClick` event) – both will fire without overriding each other.
...
Set `eventOverlapMode="cascade"` to indent overlapping events instead of splitting the available
width into columns. `eventOverlapRaiseDelay` controls how long a covered event must be hovered before
it is raised above the stack.

<Demo data={WeekViewDemos.eventOverlapMode} />

## Drag and drop

Enable drag and drop by setting `withEventsDragAndDrop` prop. Events can be dragged to different days and times.
...
Set `withExternalEventDrop` and listen for the `externalEventDrop` event to allow dragging items from outside the component into
the schedule. External items must set data in `dataTransfer` during their own `dragstart` handler.
...
Combine `withExternalEventDrop` with `withEventsDragAndDrop` to enable bidirectional
...
edge of an event to adjust its start or end time. Listen for the `eventResize` event to handle
...
They emit `eventClick` like regular events. When `withEventsResize` is also enabled, timed
background events expose resize handles and honor `canResizeEvent`. They cannot be dragged.
...
When the drag ends, the `slotDragEnd` event is emitted with the range start and end dates.
The drag is constrained to a single day column. Combined with the `timeSlotClick`, `allDaySlotClick`
and `eventClick` events, this enables a complete event creation and editing experience.
...
- The weekdays row, all-day slots row, and time slots grid each have their first element in the tab order (`:tab-index="0"`)
...
<KeyboardEventsTable
  data={[
    ...
    },
    {
      key: 'ArrowUp',
      description:
        'Focuses previous time slot (or all-day slot if on first time slot and all-day slots are enabled)',
    },
  ]}
/>
