import { DayViewDemos } from '@/demos'
...
<Demo data={DayViewDemos.eventOverlapMode} />

## Current time indicator

Set `withCurrentTimeIndicator` to display a line showing the current time.
By default, it's only shown for the current day. Set `:with-current-time-bubble="false"`
...
Set `:with-header="false"` to hide the header controls. This is useful when you want to
...
Enable drag and drop by setting `withEventsDragAndDrop` prop. Listen for the `eventDrop` event to handle
...
By default events snap to `intervalMinutes` when moved. Set `eventDragInterval` to use a
different drag step, such as 15-minute movement on a 30-minute grid.
...
unavailability, lunch breaks, focus time, or other blocked periods. By default they cannot be
...
background events expose resize handles and honor `canResizeEvent`. They cannot be dragged. An
interactive background event receives pointer interactions instead of the time slots beneath it.

<Demo data={DayViewDemos.interactiveBackgroundEvents} />
...
Control the date externally using the `date` prop and the `dateChange` event.
...
Listen for the `viewChange` event to handle view level changes when the user clicks the view selector buttons.
...
Combined with the `timeSlotClick`, `allDaySlotClick` and `eventClick` events, this enables
...
- Only the first time slot is included in the tab order (`:tab-index="0"`)
