...
Set `:with-weekend-days="false"` to remove weekend columns. Customize day contents with the
`renderDay` prop or the `day` scoped slot; both receive the date and events for that day.
