import { ScheduleDemos } from '@/demos'
...
Control the current view and date externally with `v-model:view` and `v-model:date`, or with the `viewChange` and `dateChange` events.
...
Disable the default header on all views by passing `:with-header="false"` through view-specific props.
...
- `withEventsDragAndDrop` - Enable drag and drop
- `eventDrop` - emitted when an event is dropped in a new position
- `canDragEvent` - Drag permission callback
- `withEventResize` - Enable event resizing
- `eventDragInterval` - Snap dragged events independently of the displayed time-slot interval
- `eventResizeInterval` - Snap resized events independently of the displayed time-slot interval
- `withInteractiveBackgroundEvents` - Make background events focusable and clickable
- `eventResize` - emitted when an event is resized
...
Combined with the `timeSlotClick`, `allDaySlotClick`, `dayClick` and `eventClick` events,
...
`eventDragInterval` and `eventResizeInterval` can be finer or coarser than the visible grid.
The resulting times stay within the configured schedule time range.

<Demo data={ScheduleDemos.eventIntervals} />
...
Set `withInteractiveBackgroundEvents` to make background blocks focusable and emit `eventClick`.
They remain behind regular events and are never draggable or resizable.

<Demo data={ScheduleDemos.interactiveBackgroundEvents} />
