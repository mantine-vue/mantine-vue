import { UseColorSchemeDemos } from '@/demos'
...
The `use-color-scheme` hook returns a ref with the preferred OS color scheme value (`dark` or `light`)
and subscribes to changes:

<Demo data={UseColorSchemeDemos.usage} />
...
```ts
// .value is 'dark' on server side
// .value is the computed value on client side after mount
...
Instead, the value is calculated after the component mounts.
...
The `UseColorSchemeValue` and `UseMediaQueryOptions` types are exported from the `@mantine-vue/hooks` package;
you can import them in your application:

```ts
import type { UseColorSchemeValue, UseMediaQueryOptions } from '@mantine-vue/hooks'
```
