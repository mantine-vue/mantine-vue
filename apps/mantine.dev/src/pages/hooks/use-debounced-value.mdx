import { UseDebouncedValueDemos } from '@/demos'
import { Layout } from '@/layout'
import { MDX_DATA } from '@/mdx'

export default Layout(MDX_DATA.useDebouncedValue)

## Usage

The `use-debounced-value` hook debounces value changes.
This can be useful when you want to perform a heavy operation based on reactive state,
for example, sending a search request. Unlike [use-debounced-state](/hooks/use-debounced-state/), it
is designed to work with controlled components/refs.

<Demo data={UseDebouncedValueDemos.usage} />

## Differences from use-debounced-state

- You have direct access to the non-debounced value.
- It is used for controlled inputs (`value`/`v-model` instead of an uncontrolled default), it re-renders on every change like a character typed in an input.
- It accepts a `Ref` or a getter function (`MaybeRefOrGetter<T>`), so it works with any reactive source, not just a local `ref`.

## Leading update

You can immediately update the value with the first call using `{ leading: true }` options:

<Demo data={UseDebouncedValueDemos.leading} />

## Cancel and flush

The hook returns a third element with `cancel` and `flush` handlers.
`cancel` discards the pending update, `flush` applies it immediately.
Updates cancel automatically on component unmount.

In this example, type in some text and click the cancel button
within a second to cancel debounced value change:

<Demo data={UseDebouncedValueDemos.cancel} />

The second element of the returned tuple is a shorthand for `cancel` for backwards compatibility.

```ts
const [debounced, cancel, { cancel, flush }] = useDebouncedValue(value, 200)
```

## Definition

```ts
interface UseDebouncedValueOptions {
  leading?: boolean
}

interface UseDebouncedValueHandlers {
  cancel: () => void
  flush: () => void
}

type UseDebouncedValueReturnValue<T> = [Readonly<Ref<T>>, () => void, UseDebouncedValueHandlers]

function useDebouncedValue<T>(
  value: MaybeRefOrGetter<T>,
  wait: number,
  options?: UseDebouncedValueOptions,
): UseDebouncedValueReturnValue<T>
```

## Exported types

The `UseDebouncedValueOptions` and `UseDebouncedValueReturnValue` types are exported from the `@mantine-vue/hooks` package;
you can import them in your application:

```ts
import type { UseDebouncedValueOptions, UseDebouncedValueReturnValue } from '@mantine-vue/hooks'
```
