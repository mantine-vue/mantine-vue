import { ScrollerDemos, UseScrollerDemos } from '@/demos'
...
The `useScroller` hook manages horizontal scroll behavior for a container element.
It provides scroll state refs (whether content can be scrolled in either direction),
...
interface UseScrollerReturnValue<T extends HTMLElement = HTMLDivElement> {
  /** Ref callback to attach to the scrollable container element, e.g. `:ref="ref"` */
  ref: (node: T | null) => void

  /** Ref, whether content can be scrolled towards the start */
  canScrollStart: Ref<boolean>

  /** Ref, whether content can be scrolled towards the end */
  ...
  /** Ref, `true` if the user is currently dragging the content */
  isDragging: Ref<boolean>

  /** Event handlers to bind on the scrollable container for drag functionality, e.g. `v-on="dragHandlers"` */
  dragHandlers: {
...
`UseScrollerOptions`, `UseScrollerReturnValue` and `UseScrollerScrollState` types are exported from the `@mantine-vue/hooks` package;
