...
The `useFloatingWindow` hook makes a given element draggable:
...
Use `enabled` option to enable or disable dragging. Since the hook reads option properties directly
(rather than through `toValue`), pass a `reactive` options object if you need `enabled` to update
...
interface UseFloatingWindowReturnValue<T extends HTMLElement> {
  /** Ref callback to bind to the element that should be draggable, e.g. `:ref="ref"` */
  ref: (node: T | null) => void

  /** Readonly ref with the current draggable element */
  element: Readonly<Ref<T | null>>

  /** Function to set the position of the element */
  setPosition: SetFloatingWindowPosition

  /** Readonly ref, `true` if the element is currently being dragged */
...
`UseFloatingWindowOptions` and `UseFloatingWindowReturnValue` types are exported from the `@mantine-vue/hooks` package;
you can import them in your application:

```ts
import type { UseFloatingWindowOptions, UseFloatingWindowReturnValue } from '@mantine-vue/hooks'
```
