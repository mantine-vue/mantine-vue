import { UseMaskDemos } from '@/demos'
...
`useMask` attaches real-time input masking to any `<input>` element. It formats user input
against a defined pattern and exposes both the masked display value and the raw unmasked value
as reactive refs. Bind the returned `ref` function to the target element through the `root-ref`
prop on Mantine Vue input components (or the native `ref` attribute on a plain `<input>`):
...
- `generatePattern(mode, options)` – generate a regex string for the HTML `pattern` attribute
  (`mode` is `'full'` to wrap each token in a capture group, or `'full-inexact'` for a plain
  concatenated pattern)

```ts
import { formatMask, unformatMask, isMaskComplete } from '@mantine-vue/hooks'
...
interface UseMaskReturnValue {
  /** Ref callback to attach to the input element (bind through `root-ref` on Mantine Vue inputs) */
...
The `UseMaskOptions`, `MaskState` and `UseMaskReturnValue` types are exported from the
`@mantine-vue/hooks` package; you can import them in your application:

```ts
import type { UseMaskOptions, MaskState, UseMaskReturnValue } from '@mantine-vue/hooks'
```
